...
def violinplots(df, columns, titles, labels, ticks, dim, row, col, palette, x = None, hue = None, split = False, xlabel = None):
    ...
    for c, column in enumerate(columns):
        ...
        sns.violinplot(y = column, x = x, data = df, hue = hue, split = split, orient = "v", palette = palette)
...
def barplots(df, columns, y, labels, ylabel, titles, dim, row, col, palette, estimator = "mean", errorbar = "ci", orient = "v", hue = None):
    """
    Parameters:
    -----------
    df     : dataframe source of data                   : dataframe :      :
    ...
    for c, column in enumerate(columns):
        ...
        sns.barplot(x = column, y = y, data = df, errorbar = errorbar, estimator = estimator, orient = orient, hue = hue, palette = palette)
        # Formatting the labels & axes
        plt.xlabel(f"{labels[c]}", size = 16)
        plt.ylabel(f"{ylabel}", size = 16)
        plt.xticks(size = 14)
...
def barplot(df, x, y, title, label, ylabel, ticks, dim, palette, estimator = "mean", errorbar = "ci", orient = "v", hue = None):
    """
    Parameters:
    -----------
    df     : dataframe source of the data               : dataframe :      :
    ...
    sns.barplot(x = x, y = y, data = df, orient = orient, errorbar=errorbar, estimator = estimator, palette = palette, hue = hue)
